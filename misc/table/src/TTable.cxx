...
void TTable::AddAt(const void *row, Int_t i)
{
   ...
   if (!BoundsOk("TTable::AddAt", i))
      i = 0;
  const Char_t *_row  = (const Char_t *)row;
  if (_row == 0) {
	   //std::pair <Char_t*, std::ptrdiff_t> _table(&fTable[i*fSize], fSize);
	   std::uninitialized_fill_n(&fTable[i*fSize], fSize, dummy);
   } else {
	   std::uninitialized_copy_n(_row, fSize, &fTable[i*fSize]);
   }   
...
{
   std::uninitialized_copy_n(src, fSize*fN, dest);
}
////////////////////////////////////////////////////////////////////////////////
///to be documented
...
Char_t *TTable::Create()
{
   if (!fTable) {
      ...
      std::uninitialized_fill_n(fTable, fSize*fN, 0);
   }
   return fTable;
}
   ...
   while (rowCount) {
      ...
      for (; member != dscE; member++){
         ...
         if (dim) {
            ...
            std::uninitialized_fill_n(arrayLayout, dim, 0);
         }
         Int_t arrayLength  = 1;
	 ...
	 Int_t same = 0;
		...
		if (indexOffset == arrayLength - 1) {
		  std::cout << "\t " << std::setw(strlen((*member).fColumnName));
		  for (Int_t i=0;i<dim;i++) {
		    std::cout << "["<<std::dec<<arrayLayout[i]<<"]";
		  }
		} else if (same == 1) {std::cout << "..." << std::endl; continue;
		} else continue;
	      } else {
		if (indexOffset) std::cout << "\t " << std::setw(strlen((*member).fColumnName));
		for (Int_t i=0;i<dim;i++) {
		  std::cout << "["<<std::dec<<arrayLayout[i]<<"]";
		}
		LineOld = Line.str();
...
//              Row loop
   ...
   for (rowCount=0;rowCount<rowNumber; rowCount++,startRow += fSize, nextComment.Reset()) {     //row loop
      out << "std::uninitialized_fill_n(" << "&" << rowId << ",0," << tableId << "->GetRowSize()" << ");" << std::endl ;
...
//              Add the dimensions to "array" members
      Int_t dim = (*member).fDimensions;
      if (dim) std::uninitialized_fill_n(arrayLayout, dim*sizeof(Int_t), 0);
      Int_t arrayLength  = 1;
...
//                      Actual size of char array
         ...
         out << " std::uninitialized_copy_n(&" << rowId << "." << (const char*)memberName;
         out << ",\"";
...
void TTable::Reset(Char_t c)
{
   if (fTable) {
      ResetMap(kTRUE);
      std::uninitialized_fill_n(fTable, fSize*fN, c);
